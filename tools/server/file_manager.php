...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && $action === 'list') {
    ...
    // System/hidden files to exclude from listing
    $hiddenFiles = ['manifest.json', '.htaccess', '.DS_Store', 'Thumbs.db', 'desktop.ini'];
    $hiddenExts  = ['php', 'html', 'htm', 'css', 'js', 'log', 'sh', 'bat'];

    $items = [];
    foreach (scandir($dir) as $entry) {
        if ($entry === '.' || $entry === '..') continue;
        if ($entry[0] === '.') continue; // skip hidden files
        if (in_array($entry, $hiddenFiles, true)) continue;
